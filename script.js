...
function addToCart(name, price, imgSrc) {
  if (cart[name]) {
    ...
    cart[name] = { price, imgSrc, quantity: 1 };
  }
  updateCartDisplay();
}
...
function updateCartDisplay() {
  ...
  for (const itemName in cart) {
    ...
    li.innerHTML = `
      <img src="${item.imgSrc}" alt="${itemName}" width="50" />
      <span>${itemName}</span>
      <input type="number" min="1" value="${item.quantity}" onchange="updateQuantity('${itemName}', this.value)" />
